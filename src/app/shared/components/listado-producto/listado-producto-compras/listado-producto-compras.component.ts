...
@Component({
  selector: 'app-listado-producto-compras',
  templateUrl: './listado-producto-compras.component.html',
  styleUrls: ['./listado-producto-compras.component.css']
})
export class ListadoProductoComprasComponent implements OnInit {
  ...
  listarProductosComprasGastosPorSucursal(cod_sucursal : string)
  {
    ...
    return new Promise((resolve, reject) => {
      this.productoservice.listarProductosComprasGastosPorSucursal(cod_sucursal).subscribe( (data : any) =>
      {
        this.loadinglistado = false;
        this.datosproducto = data;
        this.cantidad_registros = data.length;
  ...
  buscarcodigoproductocompras(codigo_barra : string)
  {
      try
      {
        if (this.codigosproducto == "1") {
