...
@Component({
  selector: 'app-reporte-ingresos-manuales',
  templateUrl: './reporte-ingresos-manuales.component.html',
  styleUrls: ['./reporte-ingresos-manuales.component.css']
})
export class ReporteIngresosManualesComponent implements OnInit {
  ...
  total_base_entrada : number = 0;
  total_entrada : number = 0;
  cantidad_entrada : number = 0;
  ...
  tipo_ingreso_mercaderia: string = "TODOS";
  ...
  changeTipoIngresoMercaderia(event: any): void {
    ...
    this.tipo_ingreso_mercaderia = resultado.tipo_ingreso_mercaderia;
  }
    ...
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        let obj = {
          ...
          "T. COSTO" : element.total_base_entrada,
          "T. COSTO IMP." : element.total_entrada,
          "ENTRADA" : element.cantidad_entrada
        }
        c++;
        json.push(obj);
      });
      ...
      let obj = {
        ...
        "T. COSTO" : "",
        "T. COSTO IMP." : "T. COSTO",
        "ENTRADA" : this.total_base_entrada
      }
      json.push(obj);
  ...
  exportarPdf()
  {
    if(this.datos.length>0)
    {
      ...
      titulo[8] = { text: "ENTRADA", bold: true };
      tabla.push(titulo);
      ...
      this.datos.forEach(element => {
        ...
        fila[6] = element.total_base_entrada;
        fila[7] = element.total_entrada;
        fila[8] = element.cantidad_entrada;
        tabla.push(fila);
        c++;
      });
      ...
      fila[6] = "";
      fila[7] = "T. COSTO";
      fila[8] = this.total_base_entrada;
      tabla.push(fila);
      ...
      const pdfDefinition: any = {
        ...
        content: [
          ...
          {
            text: 'Reporte de Ingresos Manuales',
            fontSize: 16,  
            alignment: 'center',  
            color: '#047886'
          },
          ...
          columns: [
                  {
                    stack: [
                      ...
                      {
                        text: 'TIPO INGRESO MERCADERÍA: ' + this.tipo_ingreso_mercaderia,
                        alignment: 'left',
                        fontSize: 11,
                        bold: true,
                        margin: [0, 0, 0, 3]
  ...
  formularioNormal(): void
  {
    ...
    this.total_base_entrada = 0;
    this.total_entrada = 0;
    this.cantidad_entrada = 0;

    this.tipo_ingreso_mercaderia = "TODOS";
  }
  ...
  listarIngresosManuales()
  {
    ...
    this.kardexservice.listarIngresosManuales(this.fechadesde, this.fechahasta, this.cod_sucursal, this.cod_categoria, this.cod_subcategoria, this.cod_producto, this.cod_tipo_ingreso_mercaderia).subscribe( (data : any) =>
    {
      ...
      if(data.length>0)
      {
        data.forEach(element => {
          this.total_base_entrada = this.total_base_entrada + parseFloat(element.total_base_entrada);
          this.total_entrada = this.total_entrada + parseFloat(element.total_entrada);
          this.cantidad_entrada = this.cantidad_entrada + parseFloat(element.cantidad_entrada);
        });
        this.total_entrada = redondeardecimales(this.total_entrada, 2);
      }
      this.datos = data;
    }, err => {
