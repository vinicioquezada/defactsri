...
import { ListarFacturasVentasComponent } from 'src/app/shared/components/venta/listar-facturas-ventas/listar-facturas-ventas.component';
...
@Component({
  selector: 'app-reporte-venta-por-producto',
  templateUrl: './reporte-venta-por-producto.component.html',
  styleUrls: ['./reporte-venta-por-producto.component.css']
})
export class ReporteVentaPorProductoComponent implements OnInit {
  multisucursal : string = "0";

  @ViewChild(ListadoEmpleadoComponent) childlistadoempleado: any;
  @ViewChild(ListadoClienteComponent) childlistadocliente: any;
  @ViewChild(ListarFacturasVentasComponent) childlistarfacturasventas: any;
    ...
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        let obj = {
          ...
          "TOTAL" : element.total_total,
          "DESCUENTO": element.total_descuento,
          "TOTAL IMP." : element.total_iva,
          "TOTAL FINAL" : element.total_total_final
        }
        json.push(obj);
      });
  ...
  exportarPdf()
  {
    if(this.datos.length>0)
    {
      let tabla = [];
      let titulo = [];
      titulo[0] = { text: "DETALLE", bold: true };
      titulo[1] = { text: "UNID. VEND.", bold: true };
      titulo[2] = { text: "TOTAL", bold: true };
      titulo[3] = { text: "DESCUENTO", bold: true };
      titulo[4] = { text: "TOTAL IMP.", bold: true };
      titulo[5] = { text: "TOTAL FINAL", bold: true };
      tabla.push(titulo);

      this.datos.forEach(element => {
        let fila = [];
        fila[0] = element.detalle;
        fila[1] = element.total_cantidad_unidad;
        fila[2] = element.total_total;
        fila[3] = element.total_descuento;
        fila[4] = element.total_iva;
        fila[5] = element.total_total_final;
        tabla.push(fila);
      });
  ...
  buscar()
  {
    ...
    this.ventaservice.listarVentasProductos(this.fechadesde, this.fechahasta, this.cod_sucursal, this.cod_usuario, this.cod_tipo_documento, this.cod_tipo_deuda, this.cod_cliente, this.cod_ruc).subscribe( (data : any) =>
    {
      ...
      if(data.length>0)
      {
        data.forEach(element => {
          element.total_total = redondeardecimales(element.total_total, 2);
          element.total_total_final = redondeardecimales(element.total_total_final, 2);
          element.total_iva = redondeardecimales(element.total_iva, 2);
          this.total = this.total + parseFloat(element.total_total);
          this.totalimpuesto = this.totalimpuesto + parseFloat(element.total_iva);
          this.totalfinal = this.totalfinal + parseFloat(element.total_total_final);
        });
  ...
  listarFacturasVentas(cod_producto: string, detalle: string, tipo_venta: string)
  {
    let item = {
      ...
      cod_tipo_deuda: this.cod_tipo_deuda,
      cod_cliente: this.cod_cliente,
      cod_ruc: this.cod_ruc,
      cod_producto: cod_producto
    }
    this.childlistarfacturasventas.detalle = detalle;
    this.childlistarfacturasventas.tipo_venta = tipo_venta;
    this.childlistarfacturasventas.listarNotaCreditoVenta(item);
    $("#mymodallistarfacturasventas").modal("show");
  }
