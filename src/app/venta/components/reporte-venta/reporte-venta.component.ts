...
export class ReporteVentaComponent implements OnInit {
  ...
  totalautorizadas: number = 0;
  cantidadautorizadas: number = 0;
  totalenproceso: number = 0;
  cantidadenproceso: number = 0;
  totalnotaventa: number = 0;
  cantidadnotaventa: number = 0;
  ...
  exportarExcelVentasAutorizacion()
  {
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        ...
        let obj = {
          ...
          "AUTORIZACIÓN" : element.claveacceso,
  ...
  async exportarPdfVentasAutorizacion()
  {
    if(this.datos.length>0)
    {
      let tabla = [];
      ...
      titulo[4] = { text: "AUTORIZACIÓN", bold: true };
      titulo[5] = { text: "FECHA", bold: true };
      titulo[6] = { text: "TIPO VENTA", bold: true };
      ...
      this.datos.forEach(element => {
        ...
        fila[4] = element.claveacceso;
        fila[5] = this.cod_tipo_fecha == "0" ? element.fecha_registro : element.fecha_hora;
        fila[6] = element.tipo_venta;
      ...
      fila[5] = "";
      fila[6] = "SUBTOTAL IMP";
      fila[7] = this.subtotalconimpuesto;
      tabla.push(fila);
  ...
  exportarExcelVentasTotales()
  {
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        ...
        let obj = {
          "FECHA" : this.cod_tipo_fecha == "0" ? moment(element.fecha_registro, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY') : moment(element.fecha_hora, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY'),
          "# DOC" : element.numero_factura,
          "TIPO" : element.tipo_venta,
          "CLIENTE" : element.cliente,
          "SUBTOTAL" : element.subtotalconimpuesto,
          "SUBTOTAL 0": element.subtotalsinimpuesto,
      ...
      let obj = {
        ...
        "TIPO" : "",
        "CLIENTE" : "TOTALES",
        "SUBTOTAL" : this.subtotalconimpuesto,
        "SUBTOTAL 0": this.subtotal0,
        "IVA" : this.totalimpuesto,
        "TOTAL" : this.total
      }
      json.push(obj);
  ...
  async exportarPdfVentasTotales()
  {
    if(this.datos.length>0)
    {
      let tabla = [];
      let titulo = [];
      titulo[0] = { text: "FECHA", bold: true };
      titulo[1] = { text: "# DOC", bold: true };
      titulo[2] = { text: "TIPO", bold: true };
      titulo[3] = { text: "CLIENTE", bold: true };
      titulo[4] = { text: "SUBTOTAL", bold: true };
      titulo[5] = { text: "SUBTOTAL 0", bold: true };
      titulo[6] = { text: "IVA", bold: true };
      titulo[7] = { text: "TOTAL", bold: true };
      tabla.push(titulo);

      this.datos.forEach(element => {
        let fila = [];
        fila[0] = this.cod_tipo_fecha == "0" ? moment(element.fecha_registro, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY') : moment(element.fecha_hora, 'YYYY-MM-DD HH:mm:ss').format('DD/MM/YYYY');
        fila[1] = element.numero_factura;
        fila[2] = element.tipo_venta;
        fila[3] = element.cliente;
        fila[4] = element.subtotalconimpuesto;
        fila[5] = element.subtotalsinimpuesto;
        fila[6] = element.total_iva;
        fila[7] = element.importetotal;
        tabla.push(fila);
      });
      ...
      fila[2] = "";
      fila[3] = "TOTALES";
      fila[4] = this.subtotalconimpuesto;
      fila[5] = this.subtotal0;
  ...
  reiniciarValores()
  {
    ...
    this.total = 0;
    this.totalautorizadas = 0;
    this.cantidadautorizadas = 0;
    this.totalenproceso = 0;
    this.cantidadenproceso = 0;
    this.totalnotaventa = 0;
    this.cantidadnotaventa = 0;
    this.totalcobrado = 0;
  ...
  buscar()
  {
    ...
    this.ventaservice.listarVentas(this.fechadesde, this.fechahasta, this.cod_sucursal, this.cod_usuario, this.cod_tipo_documento, this.cod_tipo_deuda, this.cod_cliente, this.cod_identificacion, this.cod_vendedor, codigos_ruc, this.cod_tipo_fecha, this.cod_estado_recaudacion).subscribe( (data : any) =>
    {
      ...
      if(data.length>0)
      {
        data.forEach(element => {
          ...
          if(element.tipo_venta == "ELECTRONICA" )
          {
            if(element.estado == "AUTORIZADO" )
            {
              this.totalautorizadas = this.totalautorizadas + parseFloat(element.importetotal);
              this.cantidadautorizadas++;
            }

            if(element.estado == "EN PROCESO" )
            {
              this.totalenproceso = this.totalenproceso + parseFloat(element.importetotal);
              this.cantidadenproceso++;
            }
          }
          else
          {
            this.totalnotaventa = this.totalnotaventa + parseFloat(element.importetotal);
            this.cantidadnotaventa++;
          }

          if(element.deudor==1)
          {
            this.totalporcobrar = this.totalporcobrar + parseFloat(element.importetotal);
            this.cantidadporcobrar++;
          }
          else
          {
            this.totalcobrado = this.totalcobrado + parseFloat(element.importetotal);
  ...
  clickExportarAts()
  {
    $("#mymodalats").modal("show");
  }

  descargarAtsVentas() {
    let miVentana = window.open(this.configService.settings.baseUrl + "/reportes/ventas/atsventas.php?cod_mes=" + this.cod_mes, "ATS Ventas", 'width=600,height=400,left=300,top=100');
       miVentana.focus();
  }
