...
@Component({
  selector: 'app-reporte-venta-forma-pago',
  templateUrl: './reporte-venta-forma-pago.component.html',
  styleUrls: ['./reporte-venta-forma-pago.component.css']
})
export class ReporteVentaFormaPagoComponent implements OnInit {
  ...
  totalsinutilizacion: number = 0;
  totaltcredito: number = 0;
  totaltdebito: number = 0;
  totalotros: number = 0;
  cantidadsinutilizacion: number = 0;
  cantidadtcredito: number = 0;
  cantidadtdebito: number = 0;
  ...
  exportarPdf()
  {
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        ...
        fila[4] = this.cod_tipo_fecha == "0" ? element.fecha_registro : element.fecha_hora;
      ...
      const pdfDefinition: any = {
        ...
        content: [
          ...
          {
            text: 'Reporte de Ventas por Forma de Pagos',
            fontSize: 16,  
            alignment: 'center',  
            color: '#047886'
          },
          ...
          {
            table: {
              widths: ['auto', 'auto', 'auto', 200, 'auto', 'auto', 'auto', 'auto'],
              body:
                tabla
            }
          }
  ...
  reiniciarValores()
  {
    ...
    this.total = 0;
    this.totalsinutilizacion = 0;
    this.totaltcredito = 0;
    this.totaltdebito = 0;
    this.totalotros = 0;
    this.cantidadsinutilizacion = 0;
    this.cantidadtcredito = 0;
    this.cantidadtdebito = 0;
  ...
  buscar()
  {
    ...
    this.ventaservice.listarVentasFormaPago(this.fechadesde, this.fechahasta, this.cod_sucursal, this.cod_usuario, this.cod_tipo_documento, this.cod_tipo_deuda, this.cod_cliente, this.cod_identificacion, this.cod_vendedor, this.id_forma_pago, this.cod_ruc, this.cod_tipo_fecha, this.cod_estado_recaudacion).subscribe( (data : any) =>
    {
      ...
      if(data.length>0)
      {
        data.forEach(element => {
          ...
          if(element.id_forma_pago=="01")
          {
            this.totalsinutilizacion = this.totalsinutilizacion + parseFloat(element.valor);
            this.cantidadsinutilizacion++;
          }

          if(element.id_forma_pago=="19")
          {
            this.totaltcredito = this.totaltcredito + parseFloat(element.valor);
            this.cantidadtcredito++;
          }

          if(element.id_forma_pago=="16")
          {
            this.totaltdebito = this.totaltdebito + parseFloat(element.valor);
            this.cantidadtdebito++;
          }

          if(element.id_forma_pago=="20")
          {
            this.totalotros = this.totalotros + parseFloat(element.valor);
            this.cantidadotros++;
          }

          if(element.deudor==1)
          {
            this.totalporcobrar = this.totalporcobrar + parseFloat(element.valor);
            this.cantidadporcobrar++;
          }
          else
          {
            this.totalcobrado = this.totalcobrado + parseFloat(element.valor);
