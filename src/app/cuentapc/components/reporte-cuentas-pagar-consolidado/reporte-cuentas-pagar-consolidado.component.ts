...
@Component({
  selector: 'app-reporte-cuentas-pagar-consolidado',
  templateUrl: './reporte-cuentas-pagar-consolidado.component.html',
  styleUrls: ['./reporte-cuentas-pagar-consolidado.component.css']
})
export class ReporteCuentasPagarConsolidadoComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService, private error:ErrorService, private sucursalesservice:SucursalesService, private formapagoservice : FormaPagoService, private abonocompraservice : AbonoCompraService, private usersession: UserSessionService, private bodyStyleService: BodyStyleService) { }
  ...
  exportarExcel()
  {
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        let obj = {
          "IDENTIFICACIÓN" : element.ruc,
          "PROVEEDOR": element.proveedor,
          "TOTAL COMPRA" : element.importetotal,
          "TOTAL ABONO" : element.total_abono,
          "TOTAL SALDO A PAGAR" : element.total_deuda
        }
        json.push(obj);
      });
      ...
      let obj = {
        ...
        "PROVEEDOR": "",
        "TOTAL COMPRA" : "",
        "TOTAL ABONO" : "TOTAL COMPRA",
        "TOTAL SALDO A PAGAR" : redondeardecimales(this.totalimporte, 2)
      }
      json.push(obj);
      ...
      obj = {
        ...
        "TOTAL SALDO A PAGAR" : redondeardecimales(this.totalabono, 2)
      }
      json.push(obj);
      ...
      obj = {
        ...
        "PROVEEDOR": "",
        "TOTAL COMPRA" : "",
        "TOTAL ABONO" : "TOTAL SALDO A PAGAR",
        "TOTAL SALDO A PAGAR" : redondeardecimales(this.totaldeuda, 2)
      }
      json.push(obj);
  ...
  exportarPdf()
  {
    if(this.datos.length>0)
    {
      ...
      titulo[2] = { text: "TOTAL COMPRA", bold: true };
      titulo[3] = { text: "TOTAL ABONO", bold: true };
      titulo[4] = { text: "TOTAL SALDO A PAGAR", bold: true };
      tabla.push(titulo);
      ...
      this.datos.forEach(element => {
        ...
        fila[3] = element.total_abono;
        fila[4] = element.total_deuda;
        tabla.push(fila);
      });
      ...
      fila[1] = "";
      fila[2] = "";
      fila[3] = "TOTAL IMPORTE";
      fila[4] = redondeardecimales(this.totalimporte, 2);
      tabla.push(fila);
      ...
      fila[2] = "";
      fila[3] = "TOTAL ABONO";
      fila[4] = redondeardecimales(this.totalabono, 2);
      tabla.push(fila);
      ...
      fila[1] = "";
      fila[2] = "";
      fila[3] = "TOTAL SALDO A PAGAR";
      fila[4] = redondeardecimales(this.totaldeuda, 2);
      tabla.push(fila);
      ...
      const pdfDefinition: any = {
        ...
        content: [
          ...
          {
            text: 'Reporte de Cuentas por Pagar de Compras Consolidadas por Proveedor',
            fontSize: 16,  
            alignment: 'center',  
            color: '#047886'
          },
  ...
  formularioNormal(): void
  {
    ...
    this.cod_tipo_deuda = "1";
    this.tipo_deuda = "DEUDOR";
  ...
  buscar()
  {
    ...
    this.abonocompraservice.listarCuentasPagarConsolidado(this.cod_sucursal, this.cod_usuario, this.cod_tipo_documento, this.id_forma_pago, this.cod_proveedor, this.cod_tipo_deuda).subscribe( (data : any) =>
    {
      ...
      if(data.length>0)
      {
        if(this.cod_tipo_deuda=="1")
        {
          data = data.filter(element => parseFloat(redondeardecimales(element.total_abono, 2)) < parseFloat(redondeardecimales(element.importetotal, 2)));
        }
        data.forEach(element => {
          ...
          this.totalabono = this.totalabono + parseFloat(element.total_abono);
          element.total_deuda = redondeardecimales(parseFloat(element.importetotal) - parseFloat(element.total_abono), 2);
        });
        this.totalimporte = redondeardecimales(this.totalimporte, 2);
        this.totalabono = redondeardecimales(this.totalabono, 2);
        this.totaldeuda = this.totalimporte - this.totalabono;
