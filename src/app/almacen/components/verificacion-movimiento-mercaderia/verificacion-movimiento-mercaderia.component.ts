...
@Component({
  selector: 'app-verificacion-movimiento-mercaderia',
  templateUrl: './verificacion-movimiento-mercaderia.component.html',
  styleUrls: ['./verificacion-movimiento-mercaderia.component.css']
})
export class VerificacionMovimientoMercaderiaComponent implements OnInit {
  ...
  imprimir()
  {	 
    window.open(this.configService.settings.baseUrl + "/reportes/almacen/movimientomercaderia?cod_movimiento_mercaderia=" + this.cod_movimiento_mercaderia + "&numero_movimiento=" + this.numero_movimiento, "width=800, height=500");
  }

  verificarMovimientoMercaderia(cod_movimiento_mercaderia: string)
  {	 
    this.router.navigate(["/menualmacen/verificarmercaderia/", cod_movimiento_mercaderia]);
  }
  ...
  listarMovimientosMercaderias()
  {
    ...
    this.movimientomercaderiaService.listarMovimientosMercaderiasPorSucursalUsuario(this.cod_sucursal, this.fechadesde, this.fechahasta).subscribe( (data : any) =>
    {
  ...
  {
    Swal.fire({
      title: 'ANULAR VERIFICACIÓN MOVIMIENTO Nº '  + this.numero_movimiento,
