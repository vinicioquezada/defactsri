...
export class ReporteIngresosComponent implements OnInit {
  ...
  constructor(private toastr: ToastrService, private error:ErrorService, private sucursalesservice:SucursalesService, private formapagoservice : FormaPagoService, private ingresosservice : IngresosService, private categoriaingresosservice: CategoriaIngresosService, private usersession: UserSessionService) { }
  ...
  changeCategoriaIngresos(event: any): void {
    ...
    const resultado = this.datoscategoriaingresos.find( (valor : any) => valor.cod_categoria_ingresos == this.cod_categoria_ingresos );
  ...
  exportarExcel()
  {
    if(this.datos.length>0)
    {
      ...
      this.datos.forEach(element => {
        let obj = {
          "CATEGORÍA" : element.categoria_ingresos,
          "INGRESO" : element.ingresos,
  ...
  exportarPdf()
  {
    if(this.datos.length>0)
    {
      ...
      titulo[1] = { text: "INGRESO", bold: true };
      ...
      this.datos.forEach(element => {
        let fila = [];
        fila[0] = element.categoria_ingresos;
        fila[1] = element.ingresos;
          ...
          columns: [
                  {
                    stack: [
                      ...
                      {
                        text: 'CATEGORÍA INGRESOS: ' + this.categoria_ingresos,
                        alignment: 'left',
                        fontSize: 11,
                        bold: true,
                        margin: [0, 0, 0, 3]
  ...
  buscar()
  {
    ...
    this.ingresosservice.listarIngresosPorFechas(this.fechadesde, this.fechahasta, this.cod_sucursal, this.cod_usuario, this.id_forma_pago, this.cod_categoria_ingresos).subscribe( (data : any) =>
    {
  ...
  async listarCategoriaIngresos(): Promise<void> {
    this.loadinglistado = true;
    this.datoscategoriaingresos = [];

    try {
      const data = await firstValueFrom(this.categoriaingresosservice.listarCategoriaIngresos());

      this.datoscategoriaingresos = data;

      const categoriaTodos = {
        cod_categoria_ingresos: "0",
        categoria_ingresos: "TODOS"
      };

      this.datoscategoriaingresos.unshift(categoriaTodos);
